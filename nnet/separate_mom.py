...
def run(args):
    ...
    for key, mix_samps in tqdm(mix_input):
        ...
        aviableKeys = [key for key in keys if not re.search("((("+firstSpeaker+")|("+secondSpeaker+"))?.*_.*_(("+firstSpeaker+")|("+secondSpeaker+")).*)|((("+firstSpeaker+")|("+secondSpeaker+")).*_.*_(("+firstSpeaker+")|("+secondSpeaker+"))?.*)", key)] 
        ...
        for idx, samps in enumerate(spks):
            ...
            #Saving separated speakers with name that contains first and second mixture key separated by '='
            write_wav(

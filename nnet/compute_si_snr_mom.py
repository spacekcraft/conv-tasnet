...
from libs.metric import si_snr, permute_si_snr, permute_si_snr_mix_of_mix, combine_si_snr_mix_of_mix
...
class SpeakersReader(object):
    ...
    def keys(self):
        first_reader = self.readers[0]
        return first_reader.index_keys
...
def run(args):
    ...
    if single_speaker:
        ...
        for key, sep_list in tqdm(sep_reader):
            #keys = key.split('=')
            ref_list = ref_reader[key]# + ref_reader[keys[1]]
            zero_ref_list = ref_reader[key]# + ref_reader[keys[1]]
            if args.mixofmix != 0:
            ...
            #Cut lengths
            #Find if lenghts diffs
            '''
            diff = False
            for i in range(len(sep_list)):
               if sep_list[i].size != ref_list[i].size:
                   diff = True
                   break
                '''
            if sep_list[0].size != ref_list[0].size:
                ...
                #end = 999999999999
                #for i in range(len(sep_list)):
                #    end = min(min(sep_list[i].size, ref_list[i].size), end)
            ...
            #PIT
            snr = combine_si_snr_mix_of_mix(sep_list, ref_list)
            reporter.add(key, snr)
    reporter.report()
